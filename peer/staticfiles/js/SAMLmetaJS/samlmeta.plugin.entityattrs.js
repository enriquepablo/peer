		...
		"addEntityAttr": function(entityattr) {
			...
			var entityattrHTML = '<fieldset><legend>Attribute</legend>' +
				...
				'<div class="entityattrfield">' +
					...
					'<input type="text" name="entityattr-' + randID + '-values-name" id="entityattr-' + randID + '-values" value="' + (entityattr.value || '')+ '" />' +
				'</div>' +
    ...
    SAMLmetaJS.plugins.entityattrs = {
        ...
        fromXML: function (entitydescriptor) {
			...
			if (entitydescriptor.entityAttributes) {
				for (i = 0; i < entitydescriptor.entityAttributes.length; i++ ) {
		...
		toXML: function (entitydescriptor) {
			$('div#entityattrs fieldset').each(function (index, element) {
				...
				newEntityAttr.nameFormat = $(element).find('input').eq(0).attr('value');
				newEntityAttr.name = $(element).find('input').eq(1).attr('value');
				newEntityAttr.friendlyName = $(element).find('input').eq(2).attr('value');
                newEntityAttr.values = [];
